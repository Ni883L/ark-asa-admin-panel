...
const Renderers = {
  ...
  renderStats(status = {}, metrics = {}, versions = {}) {
    ...
    const fields = {
      ...
      RAM: metrics.memory || '-',
      Disk: metrics.disk || '-',
      Ports: metrics.ports || metrics.displayPorts || status.configuredPorts || '-',
...
const App = {
  ...
  bindEvents() {
    ...
    bindServiceAction('asaServiceInstallBtn', '/api/actions/asa-service-register', 'Install', 'ARK ASA Dienst registrieren (WinSW/UAC)');
            ...
            if (action === 'backup') {
              ...
              if (action === 'panel-update') {
                ...
                try {
                  ...
                  const message = String(error.message || '');
                  if (message.includes('Netzwerkfehler: API nicht erreichbar') || message.includes('Nicht angemeldet')) {
                    UI.setFeedback('Server-Stopp wurde ausgelöst. Verbindung wird neu aufgebaut...', 'info');
                    setTimeout(() => window.location.reload(), 5000);
                    return;
                  }
                  throw error;
